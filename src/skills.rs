...
fn use_number_key_skills(
    ...
    let dir = anim.direction.normalize_or_zero();
...
fn use_dash_skill_with_ctrl(
    ...
    cooldowns.dash = (cooldowns.dash - time.delta_secs()).max(0.0);

    if keyboard.just_pressed(KeyCode::ControlLeft) && cooldowns.dash <= 0.0 {
        dash.timer = Timer::from_seconds(crate::movement::DASH_DURATION, TimerMode::Once);
        dash.active = true;
        if anim.direction == Vec2::ZERO {
            anim.direction = Vec2::Y;
        }
        cooldowns.dash = pool.def(SkillId::Dash).cooldown;
    }
}
