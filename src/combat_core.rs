...
impl Plugin for CombatCorePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<EnemyHpBarMap>()
            ...
            .add_systems(
                Update,
                (update_projectiles, update_slash_vfx, sync_enemy_hp_bars, process_enemy_death).in_set(CombatSet),
            );
    }
}
...
pub fn skill_slash(origin: Vec2, dir: Vec2, enemies_q: &mut Query<(Entity, &Transform, &mut Health), With<Enemy>>) {
...
fn update_projectiles(
    ...
    mut player_q: Query<(&Transform, &mut Health), (With<Player>, Without<Projectile>)>,
) {
...
fn sync_enemy_hp_bars(mut commands: Commands, enemies_q: Query<(Entity, &Health, &Transform), With<Enemy>>, mut bar_map: ResMut<EnemyHpBarMap>) {
